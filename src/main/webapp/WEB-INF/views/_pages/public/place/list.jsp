                        ...
                        <div>
                            <ul class="pagination justify-content-center">
                                <c:if test="${currentPage gt 1}">
                                    <li class="page-item">
                                        <a href="<c:url value="/place?p=${currentPage - 1}&s=${search}&t=${type}&a=${sort}"/>" class="page-link">이전</a>
                                    </li>
                                </c:if>
                                <c:forEach var="page" begin="${start}" end="${end}">
                                    <li class="page-item">
                                        <a href="<c:url value="/place?p=${page}&s=${search}&t=${type}&a=${sort}"/>" class="page-link <c:if test="${page eq currentPage}"> active </c:if>">${page}</a>
                                    </li>
                                </c:forEach>
                                <c:if test="${currentPage lt totalPages}">
                                    <li class="page-item">
                                        <a href="<c:url value="/place?p=${currentPage + 1}&s=${search}&t=${type}&a=${sort}"/>" class="page-link">다음</a>
                                    </li>
                                </c:if>
                            </ul>
                        </div>
